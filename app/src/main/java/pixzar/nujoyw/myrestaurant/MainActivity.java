...
public class MainActivity extends AppCompatActivity {
    ...
    public void clickLogin(View view) {
        ...
        if (strUser.equals("") || strPassword.equals("")) {
            errorDialog("ERROR","กรุณาใส่ให้ครบ");
        } else {
            try {
                String[] strMyResult = objUserTABLE.searchUser(strUser);
                if (strPassword.equals(strMyResult[2])) {
                    welcomDialog(strMyResult[3]);
                } else {
                    errorDialog("Error Password Fail", "ใส่ password อีกครั้ง");
                }
            } catch (Exception e) {
    ...
    private void welcomDialog(final String strName) {
        ...
        objBuilder.setIcon(R.drawable.phone);
        objBuilder.setTitle("Welcome Dear");
